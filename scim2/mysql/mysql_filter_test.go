...
func TestFilterConverter(t *testing.T) {
	...
	c.SetBindCounter(1)
	err := c.Convert("userName eq \"bjensen\"", map[string]string{"userName": "user_name"})
	assert.Nil(t, err)
	clause := c.GetClause()
	...
	c.SetBindCounter(1)
	err = c.Convert("meta.lastModified gt \"2011-05-13T04:42:34Z\"", map[string]string{"meta.lastModified": "last_modified"})
	assert.Nil(t, err)
	clause = c.GetClause()
	...
	c.SetBindCounter(1)
	err = c.Convert("emails[type eq \"work\" and value co \"@example.com\"] or ims[type eq \"xmpp\" and value co \"@foo.com\"]",
		map[string]string{
